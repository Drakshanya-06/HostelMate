...
const AdminDashboard = ({ students, guestRequests, rooms, onUpdateGuest, onUpdatePhoto, onUpdateGuestData, onLogout, onAddRoom, complaints, onUpdateComplaint, onRefreshData }) => {
    ...
    const simulateEmail = (req) => {
        ...
        const roomNo = rooms.find(r => (r.capacity || 0) > (r.currentOccupancy || 0))?.roomNumber || "TBD-101";
        const emailBody = `
    ...
    const handleSendReminder = () => {
        ...
        const contact = selectedStudent.email || selectedStudent.phone || 'student';
        const amount = selectedStudent.pendingFee?.toLocaleString() || '0';
        addToast(`Reminder sent to ${contact} for ₹${amount}`, 'success');
        console.log(`Reminder action: student=${selectedStudent.name}, contact=${contact}, amount=₹${amount}`);
    };
    ...
    const pendingFees = students.reduce((acc, s) => acc + (s.pendingFee || 0) + (s.messBill || 0) + (s.gymBill || 0) + (s.laundryBill || 0), 0);
    const pendingGuestRequests = guestRequests.filter(g => g.status === 'pending').length;
    const gymUsers = students.filter(s => s.hasGym || (s.gymBill === 0 && s.feeStatus === 'paid')).length;
    const gymPending = students.filter(s => (s.gymBill || 0) > 0).length;
    const laundryUsers = students.filter(s => s.hasLaundry || (s.laundryBill === 0 && s.feeStatus === 'paid')).length;
    const laundryPending = students.filter(s => (s.laundryBill || 0) > 0).length;
        ...
        <div className="min-h-screen flex flex-col md:flex-row bg-background font-sans relative text-textPrimary">
            ...
            <aside className="w-full md:w-72 bg-surface/50 backdrop-blur-3xl border-r border-white/5 md:min-h-screen sticky top-0 z-40 flex flex-col shadow-2xl shadow-black/50">
                <div className="p-8 border-b border-white/5">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="bg-primary p-2.5 rounded-2xl shadow-xl shadow-primary/30 animate-float flex items-center justify-center">
                        ...
                        <div className="leading-tight">
                            ...
                            <p className="text-[10px] font-black text-textSecondary/50 uppercase tracking-[0.3em] mt-1">Command Center</p>
                        </div>
                    </div>
                </div>
            ...
            <main className="flex-1 p-8 md:p-16 overflow-y-auto bg-background text-textPrimary relative">
                ...
                <div className="space-y-10">
                        ...
                        <div className="space-y-10 animate-in fade-in duration-500">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-8">
                                <div className="p-8 rounded-2xl bg-zinc-900 border border-border relative overflow-hidden group cursor-pointer hover:border-primary/50 transition-all">
                                    ...
                                    <div className="flex gap-10 relative z-10 flex-wrap">
                                        <div>
                                            <span className="block text-3xl font-bold">{gymUsers}</span>
                                            <span className="text-[10px] font-bold text-textSecondary uppercase tracking-widest">Active Members</span>
                                        </div>
                                        <div>
                                            <span className="block text-3xl font-bold">{gymPending}</span>
                                            <span className="text-[10px] font-bold text-textSecondary uppercase tracking-widest">Pending Dues</span>
                                        </div>
                                    </div>
                                    <button onClick={() => setShowGymModal(true)} className="mt-8 px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-bold uppercase tracking-widest transition-colors relative z-10">
                                        Manage Gym
                                    </button>
                                </div>
                                ...
                                <div className="p-8 rounded-2xl bg-zinc-900 border border-border relative overflow-hidden group cursor-pointer hover:border-secondary/50 transition-all">
                                    ...
                                    <div className="flex gap-10 relative z-10 flex-wrap">
                                        <div>
                                            <span className="block text-3xl font-bold">{laundryUsers}</span>
                                            <span className="text-[10px] font-bold text-textSecondary uppercase tracking-widest">Active Users</span>
                                        </div>
                                        <div>
                                            <span className="block text-3xl font-bold">{laundryPending}</span>
                                            <span className="text-[10px] font-bold text-textSecondary uppercase tracking-widest">Pending Bills</span>
                                        </div>
                                    </div>
                                    <button onClick={() => setShowLaundryModal(true)} className="mt-8 px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-bold uppercase tracking-widest transition-colors relative z-10">
                                        View Schedule
                                    </button>
                                </div>
                            </div>
                    ...
                    {showGymModal && (
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-background/70 backdrop-blur-sm animate-in fade-in duration-300">
                            <div className="relative w-full max-w-3xl bg-surface rounded-[2rem] shadow-2xl border border-white/10 overflow-hidden">
                                <button onClick={() => setShowGymModal(false)} className="absolute top-6 right-6 w-12 h-12 rounded-2xl bg-background flex items-center justify-center text-textSecondary hover:text-primary transition-colors">
                                    <i className="fas fa-times text-xl"></i>
                                </button>
                                <div className="p-10">
                                    <div className="flex items-center justify-between mb-8">
                                        <div>
                                            <h2 className="text-3xl font-black text-textPrimary">Gym Management</h2>
                                            <p className="text-sm text-textSecondary mt-2">Track current gym usage and payment status.</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-2xl font-black text-primary">{gymUsers}</p>
                                            <p className="text-xs uppercase tracking-widest text-textSecondary">Active Members</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                        <div className="p-6 rounded-3xl bg-background border border-border">
                                            <p className="text-xs uppercase tracking-[0.35em] text-textSecondary mb-3">Members with Gym Access</p>
                                            <p className="text-4xl font-black text-textPrimary">{gymUsers}</p>
                                        </div>
                                        <div className="p-6 rounded-3xl bg-background border border-border">
                                            <p className="text-xs uppercase tracking-[0.35em] text-textSecondary mb-3">Pending Gym Dues</p>
                                            <p className="text-4xl font-black text-textPrimary">{gymPending}</p>
                                        </div>
                                    </div>

                                    <div>
                                        <h3 className="text-xl font-bold text-textPrimary mb-4">Students Awaiting Payment</h3>
                                        <div className="grid gap-4">
                                            {students.filter(s => (s.gymBill || 0) > 0).slice(0, 6).map((student, idx) => (
                                                <div key={idx} className="flex items-center justify-between p-4 rounded-3xl bg-zinc-900 border border-border">
                                                    <div>
                                                        <p className="font-bold text-textPrimary">{student.name}</p>
                                                        <p className="text-xs text-textSecondary uppercase tracking-widest">Room {student.roomNumber}</p>
                                                    </div>
                                                    <span className="text-sm font-bold text-primary">₹{student.gymBill || 0}</span>
                                                </div>
                                            ))}
                                            {students.filter(s => (s.gymBill || 0) > 0).length === 0 && (
                                                <p className="text-sm text-textSecondary">No students currently awaiting gym payments.</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                    ...
                    {showLaundryModal && (
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-background/70 backdrop-blur-sm animate-in fade-in duration-300">
                            <div className="relative w-full max-w-3xl bg-surface rounded-[2rem] shadow-2xl border border-white/10 overflow-hidden">
                                <button onClick={() => setShowLaundryModal(false)} className="absolute top-6 right-6 w-12 h-12 rounded-2xl bg-background flex items-center justify-center text-textSecondary hover:text-primary transition-colors">
                                    <i className="fas fa-times text-xl"></i>
                                </button>
                                <div className="p-10">
                                    <div className="flex items-center justify-between mb-8">
                                        <div>
                                            <h2 className="text-3xl font-black text-textPrimary">Laundry Schedule</h2>
                                            <p className="text-sm text-textSecondary mt-2">Review laundry usage and pending service fees.</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-2xl font-black text-secondary">{laundryUsers}</p>
                                            <p className="text-xs uppercase tracking-widest text-textSecondary">Active Users</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                        <div className="p-6 rounded-3xl bg-background border border-border">
                                            <p className="text-xs uppercase tracking-[0.35em] text-textSecondary mb-3">Active Laundry Users</p>
                                            <p className="text-4xl font-black text-textPrimary">{laundryUsers}</p>
                                        </div>
                                        <div className="p-6 rounded-3xl bg-background border border-border">
                                            <p className="text-xs uppercase tracking-[0.35em] text-textSecondary mb-3">Pending Laundry Bills</p>
                                            <p className="text-4xl font-black text-textPrimary">{laundryPending}</p>
                                        </div>
                                    </div>

                                    <div>
                                        <h3 className="text-xl font-bold text-textPrimary mb-4">Pending Laundry Fees</h3>
                                        <div className="grid gap-4">
                                            {students.filter(s => (s.laundryBill || 0) > 0).slice(0, 6).map((student, idx) => (
                                                <div key={idx} className="flex items-center justify-between p-4 rounded-3xl bg-zinc-900 border border-border">
                                                    <div>
                                                        <p className="font-bold text-textPrimary">{student.name}</p>
                                                        <p className="text-xs text-textSecondary uppercase tracking-widest">Room {student.roomNumber}</p>
                                                    </div>
                                                    <span className="text-sm font-bold text-secondary">₹{student.laundryBill || 0}</span>
                                                </div>
                                            ))}
                                            {students.filter(s => (s.laundryBill || 0) > 0).length === 0 && (
                                                <p className="text-sm text-textSecondary">No pending laundry fees right now.</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-4">
                                                        <div className="flex -space-x-3">
                                                            {[...Array(room.currentOccupancy || 0)].map((_, i) => (
                                                            ...
                                                            {[...Array((room.capacity || 0) - (room.currentOccupancy || 0))].map((_, i) => (
                                                        ...
                                                        <span className="text-xs font-normal text-slate-400">{room.currentOccupancy || 0}/{room.capacity}</span>
                                                    </div>
                                                </td>
                                    ...
                                    <div>
                                        ...
                                        <div className="p-6 rounded-[2rem] bg-background text-white shadow-xl relative overflow-hidden group">
                                            ...
                                            <div className="mt-8 flex gap-3">
                                                <button onClick={handleSendReminder} className="flex-1 bg-surface/10 hover:bg-surface/20 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Send Reminder</button>
