...
const LoginPage = () => {
    return (
        <div className="flex flex-col md:flex-row min-h-screen w-full overflow-hidden bg-background text-textPrimary">
            ...
            <div className="w-full md:w-2/5 bg-slate-950 text-white relative overflow-hidden flex flex-col justify-center p-12 lg:p-20 border-r border-white/10">
                <div 
                ...
                />
                <div className="absolute inset-0 z-0 bg-gradient-to-tr from-slate-950 via-slate-900/95 to-transparent" />
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 md:p-16 relative bg-background">
                <div className="w-full max-w-4xl space-y-16 relative z-10">
                    <div className="text-center md:text-left space-y-4">
                        <h3 className="text-3xl font-black font-heading text-white tracking-tight">System Access</h3>
                        <p className="text-textSecondary text-lg font-medium">Select your portal to authenticate into the environment.</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            { to: '/student-login', icon: Activity, title: 'Resident', desc: 'Personal dashboard, bills & status.' },
                            { to: '/admin-login', icon: ShieldCheck, title: 'Warden', desc: 'Central system & resident control.' },
                            { to: '/guest-login', icon: CreditCard, title: 'Guest', desc: 'Stay requests & digital tracking.' }
                        ].map((card, i) => (
                            <Link key={i} to={card.to} className="group p-10 rounded-[2.5rem] bg-surface/70 border border-white/10 hover:bg-surface/90 transition-all duration-500 text-center hover:-translate-y-2 shadow-2xl shadow-black/20">
                                <div className="w-20 h-20 mx-auto mb-8 bg-background/70 rounded-3xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-500">
                                    <card.icon className="w-10 h-10 text-primary transition-colors" />
                                </div>
                                <h4 className="text-xl font-black text-white mb-3 italic">{card.title}</h4>
